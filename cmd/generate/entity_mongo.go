...
func getEntityMongoFileName(dir, name string) string {
	fullname := fmt.Sprintf("%s/internal/app/model/impl/mongo/entity/%s.entity.go", dir, helper.ToLowerUnderlinedNamer(name))
	return fullname
}
...
func genMongoEntity(ctx context.Context, pkgName, dir, name, comment string, fields ...entityMongoField) error {
	...
	for _, field := range tfields {
		...
		if field.Type == "" {
			...
			buf.WriteString(fmt.Sprintf(`bson:"%s"`, helper.ToLowerUnderlinedNamer(field.Name)))
		}
		buf.WriteByte('`')
...
func (a Schema{{.Name}}) To{{.Name}}() *{{.Name}} {
	item := new({{.Name}})
	helper.StructMapToStruct(a, item)
	return item
}
