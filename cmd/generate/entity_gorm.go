...
func getEntityGormFileName(dir, name string) string {
	fullname := fmt.Sprintf("%s/internal/app/model/gormx/entity/%s.entity.go", dir, helper.ToLowerUnderlinedNamer(name))
	return fullname
}
...
func genGormEntity(ctx context.Context, pkgName, dir, name, comment string, fields ...entityGormField) error {
	...
	for _, field := range tfields {
		...
		gormTag := fmt.Sprintf("column:%s;", helper.ToLowerUnderlinedNamer(field.Name))
...
type {{.Name}} struct {
	...
	IsDel bool {{.BackQuote}}gorm:"column:is_del;default:false;index;not null;"{{.BackQuote}}
