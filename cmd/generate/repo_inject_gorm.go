...
func getRepoInjectGormFileName(dir string) string {
	fullname := fmt.Sprintf("%s/internal/app/model/gormx/repo/main.go", dir)
	return fullname
}

// 插入model注入文件
func insertRepoInjectGorm(ctx context.Context, dir, name string) error {
	fullname := getRepoInjectGormFileName(dir)
