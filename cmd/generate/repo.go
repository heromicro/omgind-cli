...
func getModelFileName(dir, name string) string {
	fullname := fmt.Sprintf("%s/internal/app/model/%s.repo.go", dir, helper.ToLowerUnderlinedNamer(name))
	return fullname
}

// 生成model文件
func genRepo(ctx context.Context, pkgName, dir, name, comment string) error {
